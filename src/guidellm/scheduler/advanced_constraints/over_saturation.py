...
class OverSaturationDetector(OverSaturationDetectorBase):
    def __init__(
        ...
        self.minimum_ttft = minimum_ttft * 1000
    ...
    def check_alert(self) -> bool:
        ...
        if (
            ...
            or (self.duration < self.minimum_ttft / 1000)
...
class _OverSaturationBase(StandardBaseModel):
    """Base configuration for over-saturation constraints."""
...
class OverSaturationConstraint(Constraint):  # type: ignore[misc]
    ...
    def __call__(
        ...
        if _request_info.status == "in_progress":
            ...
            and _request_info.request_timings.request_end
        ):
            ttft = (
                _request_info.request_timings.request_end
        ...
        return SchedulerUpdateAction(
            request_queuing="stop" if is_over_saturated else "continue",
            request_processing="stop_all" if is_over_saturated else "continue",
            metadata={
...
@ConstraintsInitializerFactory.register("stop_over_saturated")
class OverSaturationConstraintInitializer(_OverSaturationBase):
    """Factory for creating OverSaturationConstraint instances from configuration."""

    @classmethod
    def from_simple_value(cls, value: bool) -> "OverSaturationConstraintInitializer":
        """
        Create an OverSaturationConstraintInitializer from a simple boolean value.

        :param value: Boolean value indicating whether to enable over-saturation detection.
        :return: Configured OverSaturationConstraintInitializer instance.
        """
        if not isinstance(value, bool):
            raise ValueError("Over-saturation constraint requires a boolean value")
        if not value:
            raise ValueError("Over-saturation constraint must be enabled (True)")
        return cls()
